...
export function TrayPopup() {
  ...
  if (loading || !data) {
    return (
      <div style={{ width: 272, height: 120, background: "#161616", borderRadius: 4, border: "0.5px solid rgba(255,255,255,0.1)", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "-apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif" }}>
  ...
  return (
    <div style={{ width: 272, background: "#161616", borderRadius: 4, border: "0.5px solid rgba(255,255,255,0.1)", fontFamily: "-apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif", overflow: "hidden", userSelect: "none" }} onMouseDown={(e) => e.stopPropagation()}>
      {data.active_account && (
        ...
        {sortedAccounts.map((account) => {
          ...
          return (
            <div key={account.id} onMouseEnter={() => setHoveredId(account.id)} onMouseLeave={() => setHoveredId(null)} onClick={() => !isSwitching && handleSwitch(account.id)}
              style={{ display: "flex", alignItems: "center", justifyContent: "space-between", height: 32, padding: "0 14px", margin: "3px 6px", borderRadius: 4, cursor: isSwitching ? "wait" : "pointer", background: isHovered ? "rgba(255,255,255,0.05)" : "transparent", transition: "background 0.12s ease", opacity: isSwitching ? 0.5 : 1 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 8, flex: 1, minWidth: 0 }}>
                ...
                {isBest && <span style={{ fontSize: 8, fontWeight: 600, padding: "0 4px", borderRadius: 4, background: "rgba(74,222,128,0.15)", color: "#4ade80", textTransform: "uppercase", letterSpacing: "0.04em", flexShrink: 0 }}>best</span>}
              </div>
              <div style={{ display: "flex", alignItems: "center", gap: 6, flexShrink: 0 }}>
                {!isHovered ? (
                  <>
                    ...
                    <span style={{ fontSize: 10, fontFamily: "ui-monospace, SFMono-Regular, Menlo, monospace", color: "rgba(255,255,255,0.72)", width: 26, textAlign: "right" }}>{formatPercent(primary)}</span>
                  </>
                ) : (
